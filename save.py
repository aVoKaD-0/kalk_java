p = {"koren":"str", "c":"int", "f":"int", "g":"int", "c2":"int", "f2":"int", "g2":"int", "a":"str", "Drob_Ne":"int", "Drob_V":"int", "number":"double", "number2":"double", "numerator":"str", "number3":"double", "number4":"double", "denominator":"str"}
d = ["koren", "c", "f", "g", "c2", "f2", "g2", "a", "Drob_Ne", "Drob_V", "number", "number2", "number3", "number4", "numerator", "denominator"]
s2 = []
    ...
    for i in range(len(lines)):
        ...
        while t < dlin:
            for h in d:
                ...
                if flag == False:
                        break
            if lines[i][t:t+3] == "str" and lines[i][t+1] == "(":
                if p[n] == "double":
                ...
                if p[n] == "int":
                    ...
                    if len(s1) > 0 and s1[0] > t:
                        s1[0] = s1[0] + 13
            if lines[i][t:t+5] == "float" and lines[i][t+1] == "(":
                if p[n] == "str":
                ...
                if p[n] == "int":
                    ...
                    if len(s1) > 0 and s1[0] > t:
                        s1[0] = s1[1] + 2
            if lines[i][t:t+3] == "int" and lines[i][t+1] == "(":
                if p[n] == "double":
                ...
                if p[n] == "str":
                    ...
                    if len(s1) > 0 and s1[0] > t:
                        s1[0] = s1[0] + 13
            if lines[i][t:t+4] == "find" and lines[i][t+1] == "(":
            ...
            if lines[i][t:t+3] == "len" and f == True  and lines[i][t+1] == "(":
                f = False
                so = t+4
            if lines[i][t] == "(" and f == False:
                s1.append(t)
            if f == False and len(s1) == 1 and lines[i][t] == ")":
                lines[i] = lines[i][:s1[0]-3] + lines[i][s1[0]:t+1] + ".length()" + lines[i][t+1:]
                s1 = []
                f = True
            if lines[i][t] == ")" and f == False:
                s1 = s1[:-1]
            ...
            if lines[i][t:t+3] == "for" and lines[i][t+3] == " ":
                lines[i] = lines[i][:t+3] + " (int " + lines[i][t+4] + "; " + lines[i][t+4] + " <" + lines[i][lines[i].find(",")+1:lines[i].find(")")+1] + "; " + lines[i][t+4] + "++):"
            ...
            if lines[i][t] == "]" and k != 0:
                if lines[i][k+1] == ":":
                    ...
                    print("d")
                    lines[i] = lines[i][:k] + ".indexOf(" + lines[i][k+1:t] + ")" + lines[i][t+1:]
            
            # print(t, dlin, lines[i])
            dlin = len(lines[i])
            t += 1
        ...
        if lines[i][-1] != ":":
            if lines[i].find("#") == -1:
                ...
                lines[i] = lines[i][:lines[i].find("#")] + ";" + lines[i][lines[i].find("#")+1:]
        elif lines[i][-1] == ":":
            lines[i] = lines[i][:-1] + " {"
